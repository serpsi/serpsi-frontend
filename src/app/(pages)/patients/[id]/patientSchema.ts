...
const CpfSchema = z.object({
	_cpf: z.string().regex(/\d{3}\.\d{3}\.\d{3}-\d{2}/)
});
...
const ParentSchema = PersonSchema.omit({
	address: true,
	_profilePicture: true
});
...
const MedicineSchema = z.object({
	...
	_dosage: z.coerce.number().positive("A dosagem deve ser maior que zero"),
	_dosageUnity: z.string().min(1, "Unidade de dosagem é obrigatória"),
	_frequency: z.coerce
		.number()
		.positive("A frequência deve ser maior que zero"),
	_firstTimeOfTheDay: z.string().min(1, "Horário é obrigatório"),
	_startDate: z
		.preprocess((val) => {

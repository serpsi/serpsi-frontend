...
export function MedicamentDialog({ triggerButton, onSuccess, medicament, patientId }: medicamentDialogProps) {
  ...
  function handleDelete(data: MedicineData) {
    toast.promise(
      deleteMedicament(patientId, (data._medicine_id || data._medicine._id?._id) as string),
      {
        ...
        <DialogContent className="lg:w-[60%]">
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              handleSubmit(onSubmit, (err) => console.log(err))(event);
            }}>
            ...
            <div className="flex flex-col gap-3">
              <div>
                <label htmlFor={`data-inicio-medicamento`} className="mb-1 block text-sm text-primary-950">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      className={`w-full rounded-md border p-2 text-left ${errors._startDate
                        ?.message ? "border-red-500 ring-red-600" : "border-primary-600 ring-primary-600"
                        }`}
                    />
                  )}
                />
                {errors._startDate
                  ?.message && <span className="text-sm text-red-500">{errors._startDate
                    ?.message}</span>}
              </div>
              <div>
                <label htmlFor={`hora-inicio-medicamento`} className="mb-1 block text-sm text-primary-950">
                    ...
                    <Input
                      ...
                      value={
                        field.value && moment(field.value).isValid()
                          ? moment.utc(field.value).format("HH:mm")
                          : ""
                      }
                      onChange={(e) => {
                        const localTime = moment.utc(e.target.value, "HH:mm").toISOString();
                ...
                />
                {errors._firstTimeOfTheDay
                  ?.message && <span className="text-sm text-red-500">{errors._firstTimeOfTheDay
                    ?.message}</span>}
              </div>
              <div>
                <InputText
